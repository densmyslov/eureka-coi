...
class CognitoIdentityProviderWrapper:
    ...
    def sign_up_user(self, given_name, family_name, email, locale, password):
        """
    ...
    def sign_in_user(self, email, password):
        ...
        try:
            ...
            response = self.cognito_idp_client.initiate_auth(**kwargs)
                    # Check if Cognito returns a Challenge
            if "ChallengeName" in response and response["ChallengeName"] == "NEW_PASSWORD_REQUIRED":
                logger.info(f"User {email} must set a new password.")
                # Return a special object telling your app to show the "set new password" flow
                return {
                    "status": "NEW_PASSWORD_REQUIRED",
                    "session": response["Session"],
                    "challenge_parameters": response.get("ChallengeParameters", {})
                }
            # Otherwise, normal login succeeded
            access_token = response["AuthenticationResult"]["AccessToken"]
            logger.info(f"User {email} was successfully authenticated.")
            return {
    ...
    def respond_to_new_password_challenge(self, email, new_password, session):
        """
        Responds to the NEW_PASSWORD_REQUIRED challenge during sign in.
        """
        try:
            kwargs = {
                'ClientId': self.client_id,
                ...
                'ChallengeResponses': {
                    ...
                    'userAttributes.given_name': 'UserFirstName',   # <-- Add these
                    'userAttributes.family_name': 'UserLastName'    # <-- Add these
                }

            }
            if self.client_secret is not None:
                kwargs['ChallengeResponses']['SECRET_HASH'] = self._secret_hash(email)

            response = self.cognito_idp_client.respond_to_auth_challenge(**kwargs)
            return response
        except ClientError as err:
            logger.error(
                "Couldn't respond to new password challenge for user %s. Here's why: %s: %s",
                email, err.response['Error']['Code'], err.response['Error']['Message'])
            raise
